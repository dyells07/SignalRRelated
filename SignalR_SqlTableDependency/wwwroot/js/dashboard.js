...
function BindSalesToGrid(sales) {
	...
	var totalRow = $('<tr/>');
	totalRow.append('<td colspan="2">Total Amount:</td>');
	totalRow.append(`<td style="background-color: #f2f2f2; font-weight: bold;">${totalAmount.toFixed(2)}</td>`); // Apply inline styles here
	totalRow.append('<td></td>');
	$('#tblSale').append(totalRow);
}
